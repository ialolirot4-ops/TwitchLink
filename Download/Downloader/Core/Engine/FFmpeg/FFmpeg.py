...
class FFmpeg(QtCore.QObject):
    ...
    def start(
        ...
        # Build -metadata key=value pairs (one pair = two list items)
        metadataArgs: tuple[str, ...] = ()
        if metadata:
            for key, value in metadata.items():
                if value:
                    metadataArgs += ("-metadata", f"{key}={value}")
        self._process.start(
            Config.PATH,
            (
                "-y",
                ...
                logLevel.value,
                "-stats",
                *(() if trimFrom is None else ("-ss", str(trimFrom))),
                *(() if trimTo is None else ("-to", str(trimTo))),
                "-i",
                "pipe:0",
