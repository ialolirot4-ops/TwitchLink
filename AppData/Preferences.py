...
class Favorites(Serializable):
    """
    Persistencia de canales favoritos y criterio de ordenamiento.
    __setup__() no toca App.FavoritesManager porque es invocado por
    Decoder.__load__() en medio de la deserializacion. El manager se
    inicializa desde apply_to_manager(), llamado despues del decode.
    """
    ...
    def set_sort(self, sort_value: str) -> None:
    ...
    def apply_to_manager(self) -> None:
        ...
        from Core import App
        from Services.Favorites.FavoritesManager import SortCriteria
        try:
            App.FavoritesManager.load_from_data(self._channels)
...
class Preferences(QtCore.QObject):
    ...
    def load(self) -> None:
        ...
        try:
            ...
            # Inicializar FavoritesManager DESPUES de que el decode haya terminado
            self.favorites.apply_to_manager()
